...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        # camera button
        camera_button = customtkinter.CTkButton(sidebar, font=custom_font,text="Camera_Test", command=lambda: self.on_tab_selected("camera_test") )
        camera_button.pack(pady=10, fill="x")

        # show cracks button
        show_cracks_button= customtkinter.CTkButton(sidebar, font=custom_font,text="show_cracks", command=lambda: self.on_tab_selected("show_cracks") )
        show_cracks_button.pack(pady=10, fill="x")
        ...
        self.camera_frame = customtkinter.CTkFrame(self.content_frame)
        self.show_cracks_frame = customtkinter.CTkFrame(self.content_frame)

        tab1 = self.train_frame 
        tab2 = self.test_frame
        tab3 = self.camera_frame
        tab4 = self.show_cracks_frame
        # Initially select the train tab
        self.on_tab_selected("train")
        ...
        self.LOAD_W_1TEST_BTN = customtkinter.CTkButton(tab2, font=custom_font,text="LOAD WEIGHT",command=self.laod_wieght)
        ...
        config_frame5 = ttk.LabelFrame(tab2,style="Custom.TLabelframe",text="THRUSHOLD  ")
        ...
        self.persetage_label=ttk.Label(config_frame5, style="Custom.TLabel",text=str(self.slider_value)+" %")
        self.persetage_label.grid(row=1, column=0, padx=100, pady=5, sticky="nsew")
        ...
        # self.thrushold_btn.grid(row=8, column=4, padx=(10, 10), pady=(10, 10), sticky="w")
        # -----------
        self.slider_1 = customtkinter.CTkSlider(config_frame5, from_=0, to=200,  number_of_steps=100, command=self.slider_changed,state='disabled')
        # self.slider_1 = customtkinter.CTkSlider(config_frame5, from_=0, to=100,  number_of_steps=100, command=self.slider_changed)
        self.slider_1.grid(row=2, column=0, padx=(20, 10), pady=(10, 10), sticky="w")
        self.slider_1.set(self.slider_value)
        # ---------------
        ...
        #============ `Grid 4` ============#
        self.button_camera = customtkinter.CTkButton(tab3, text="Camera", command=self.start_camera)
        self.button_camera.grid(row=0, column=0, padx=10, pady=10, sticky="nw")

        # 비디오 스트림 레이블 배치
        self.video_label = tk.Label(tab3)
        self.video_label.grid(row=0, column=1, columnspan=2, sticky="nsew", padx=50, pady=50)
        self.button_load_model = customtkinter.CTkButton(tab3, text="Model_Load", command=self.load_model)
        self.button_load_model.grid(row=1, column=0, padx=10, pady=10, sticky="nw")
        ...
        #=====================Grid 5================
        # anomaly map 이미지 + 원본 이미지 show
        
        self.bad_button = customtkinter.CTkButton(tab4, text="Bad", command=self.load_bad_images)
        self.bad_button.grid(row=0, column=0, padx=10, pady=10 ,sticky="nw")

        self.good_button = customtkinter.CTkButton(tab4, text="Good", command=self.load_good_images)
        self.good_button.grid(row=0, column=1, padx=10, pady=10 ,sticky="nw")

        self.image_label = customtkinter.CTkLabel(tab4)
        self.image_label.grid(row=1, column=0, sticky="nw", padx=50, pady=50)

        
        self.next_image_button = customtkinter.CTkButton(tab4, text="Next", command=self.next_image)
        self.next_image_button.grid(row=2, column=0, padx=10, pady=10 ,sticky="nw")
    ...
    def load_model(self):
        weight_path = filedialog.askopenfilename(filetypes=[("PyTorch Model", "*.pt"), ("All Files", "*.*")])
        ...
        if weight_path.endswith('.pt'):
            ...
            conv3x3_only = True
            batch_size = 16
            self.model = FastFlow(backbone_name, flow_steps, input_size,conv3x3_only)
            # 저장된 모델의 상태 로드
            checkpoint = torch.load(weight_path, map_location='cpu')
            print(checkpoint.keys())  # 체크포인트 파일의 최상위 키들을 출력합니다.
            # 모델 상태 딕셔너리 적용 (이 부분은 모델 저장 방식에 따라 달라질 수 있음)
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            if 'preds' in predictions:
                status = self.determine_status(predictions['preds'][0])
                self.display_status(frame, status)
    ...
    #_________________________________승훈추가~~~시작______________________
    # def update_frame(self):
    #     ret, frame = self.cap.read()
    #     if ret:
    #         processed_frame = self.preprocess_frame(frame)
    ...
    #             # 프레임별 tensor 값들을 저장하여 평균 계산에 활용
    #             self.frames_for_threshold.append(predictions['preds'][0])
    ...
    #     else:
    #         self.cap.release()
    #_________________________________승훈추가~~~끝______________________

    #________________기존코드!!!__________________________________
    def determine_status(self, tensor):
        # 여기서는 tensor 값에 따라 상태를 결정하는 간단한 로직을 구현합니다.
        # 실제로는 모델의 출력을 해석하여 상태를 결정해야 합니다.
        # 예시: tensor 값이 임계값 이상이면 'Bad', 아니면 'Good'
        threshold = 3000000
        status = 'Bad' if tensor.item() < threshold else 'Good'
        return status, tensor.item()
    
    #_____________________________________승훈추가~~시작_____________
    # def determine_status(self, tensor):
    #     # frames_for_threshold 리스트에 있는 모든 tensor 값들의 평균 계산
    #     if self.frames_for_threshold:
    #         average_tensor = torch.stack(self.frames_for_threshold).mean()
    #         threshold = average_tensor.item()  # 업데이트된 threshold
    #     else:
    #         # 초기 threshold 설정
    #         threshold = 3000000
    ...
    #     status = 'Bad' if tensor.item() < threshold else 'Good'
    #     return status, tensor.item()
    #______________________________________승훈추가~~끝___________


    def display_status(self, frame, status):
        # 상태를 프레임에 텍스트로 표시
        cv2.putText(frame, f'Status: {status}', (10, 50),
        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0) if status == 'Good' else (0, 0, 255), 2)


            
    def preprocess_frame(self, frame):
        """ 웹캠 프레임을 모델의 입력으로 전처리합니다. """
    ...
    def load_bad_images(self):
        self.image_paths = [os.path.join("C:/Users/Kangs/mip_pj_frontier/fastflow_from_h_notuse/FastFlow/results/2048_divide_1/pill_results/bad", filename) 
                            for filename in os.listdir("C:/Users/Kangs/mip_pj_frontier/fastflow_from_h_notuse/FastFlow/results/2048_divide_1/pill_results/bad")]
        self.display_image()

    def load_good_images(self):
        self.image_paths = [os.path.join("C:/Users/Kangs/mip_pj_frontier/fastflow_from_h_notuse/FastFlow/results/2048_divide_1/pill_results/good", filename) 
                            for filename in os.listdir("C:/Users/Kangs/mip_pj_frontier/fastflow_from_h_notuse/FastFlow/results/2048_divide_1/pill_results/good")]
        self.display_image()
    ...
    def slider_changed(self,event=None):
        print(event)
        threshold_value=float(consts.THRESHOLD)
        self.slider_value=event
        val = self.convert_to_percentage(threshold_value,event,False)
        print('%',event)
        print('convert_to_percentage --->', val)
    
    def convert_to_percentage(self,value,event,new_value):
        try:
            if not new_value:
                self.persetage_label.config(text=str(event)+" %")
            else:
                print('the old threshold is :---->',value)
                print('the new threshold is :---->',float(value)*float(event)/100)

            persentage=float(value)*float(event)/100
            # Assuming the value is a float between 0 and 1
            return persentage
        except ValueError:
    ...
    def setup_progress_window(self,total_epochs):
        ...
        total_epochs = total_epochs +1 # Example total epochs
        self.progres_bar=ttk.LabelFrame(self.train_frame ,style="Custom.TLabelframe",text="Training Progress")
        
        self.progress = ttk.Progressbar(self.progres_bar, orient="horizontal", mode='determinate', maximum=total_epochs)

        self.progress.grid(row=0, column=0 , sticky="nsew", padx=10, pady=10)

        self.progres_bar.grid(row=2, column=1, columnspan=4, sticky="nsew", padx=10, pady=10)
        # Configure the grid to allocate extra space to the LabelFrame's row and column
        self.progres_bar.grid_rowconfigure(0, weight=1)  # Make the row inside progres_bar expandable
        self.progres_bar.grid_columnconfigure(0, weight=1)  # Make the column inside progres_bar expandable
    ...
    # 모든 프레임을 숨깁니다.
        ...
        self.camera_frame.pack_forget()  # camera_frame도 숨기기 위해 추가
        self.show_cracks_frame.pack_forget()
        
        # 선택된 탭에 따라 해당 프레임만 표시합니다.
        if tab_name == "train":
            self.train_frame.pack(fill="both", expand=True)
        elif tab_name == "test":
            self.test_frame.pack(fill="both", expand=True)
        elif tab_name == "camera_test":
            # camera_test 탭 선택 시 camera_frame만 표시
            self.camera_frame.pack(fill="both", expand=True)
        elif tab_name == "show_cracks":
            self.show_cracks_frame.pack(fill="both", expand=True)
    ...
    def initialisation_folder(self):
        ...
        self.update_constant_file('IMAGE_PREDECTION_PATH',os.path.join(self.current_dir, "waiting_1.png"))
        ...
        # self.update_constant_file('THRESHOLD', '-2618053.66') 
        self.update_constant_file('THRESHOLD', '_') 
        print('slider_value befor : ',self.slider_value)
        self.slider_value=100
        print('slider_value after : ',self.slider_value)
    ...
    def start_training(self):
        ...
        process = subprocess.Popen([
                "python",
                "C:/Users/Kangs/mip_pj_frontier/fastflow_from_h/FastFlow/main.py",
                "--train"
            ])
    ...
    def start_test_All_images(self):
        ...
        self.reload_constants()
        if consts.THRESHOLD == '_'  or not consts.THRESHOLD:
            messagebox.showerror("THRESHOLD NOT VALIDE", "Please calculate the THRESHOLD")
        ...
        # Start main.py script as a subprocess and append it to the list
                new_th_value= self.convert_to_percentage(float(consts.THRESHOLD),self.slider_value,True)
                process = subprocess.Popen([
                    "python",
                    "C:/Users/Kangs/mip_pj_frontier/fastflow_from_h/FastFlow/main.py",
                    "--eval_All ",
                    "--threshold",str(new_th_value)
                ])
                self.inspection_processes.append(process)
    ...
    def start_test_All_images2(self):

        print('//// we are in START MULTIPLE TEST  section /////')
        ...
        # if not hasattr(consts, 'MULTIPLE_TEST_PATH') or consts.MULTIPLE_TEST_PATH == '_':
        response = messagebox.askyesno("TEST Folder Path Required", "The TEST Folder path is not set. Do you want to select a dataset path now?")
        if response:
            folder_selected = filedialog.askdirectory()
            self.update_constant_file('MULTIPLE_TEST_PATH', folder_selected)
            print('path of data test is :',folder_selected)
            flag=self.folder_contains_images(folder_selected)
            if flag:
                print('start multiple testtttttttt-------------')
                self.update_value(self.constantPath,'SCORE_ALL','_')
        ...
        # Start main.py script as a subprocess and append it to the list
                ...
                self.reload_constants()
                self.content_frame.configure(cursor="")
                messages = F"SCORE IS : {consts.SCORE_ALL}"
                messagebox.showinfo("Accurecy SCORE ", messages)                   
        else:
            self.content_frame.configure(cursor="")
            return 
...
#calc threshold
    ...
    def start_test_single(self):
        ...
        # Start main.py script as a subprocess and append it to the list
            new_th_value= self.convert_to_percentage(float(consts.THRESHOLD),self.slider_value,True)
            print("new_th_value:", new_th_value, type(new_th_value))
            print("test_image_path:", test_image_path, type(test_image_path))
            process = subprocess.Popen([
                "python",
                "C:/Users/Kangs/mip_pj_frontier/fastflow_from_h/FastFlow/main.py",
                "--eval_One",
